...
import { changeFacultyStatusAction, type FacultyFormState } from "../actions";

/**
 * Publish / unpublish / archive controls for a faculty member.
 *
 * Hidden from roles lacking the capability, but that is cosmetic:
 * changeFacultyStatusAction calls requireCapability() server-side on every
 * submission, so a crafted POST is refused regardless of what was rendered.
 */
...
export function FacultyStatusControls({
  facultyId,
  status,
  canPublish,
  canArchive,
  ...
  if (!canPublish && !canArchive) {
    return (
      <p className="text-xs leading-relaxed text-muted">
        A Content Admin publishes and archives mentors. You can write and save
        changes here.
      </p>
    );
  }
          ...
          {status === ContentStatus.PUBLISHED ? (
            <form action={formAction}>
              ...
              <p className="mt-2 text-xs text-muted">
                Removes this mentor from the website.
              </p>
            </form>
          ) : confirmingPublish ? (
        ...
        <div className="border-t border-brand-navy/10 pt-4">
          {status === ContentStatus.ARCHIVED ? (
            ...
            <div className="rounded-msc-lg border border-warning/25 bg-amber-50/60 px-5 py-4">
              ...
              <p className="mt-1 text-sm text-ink/75">
                This mentor will be moved to the archive and removed from the
                website. Nothing is deleted — you can restore them at any time.
              </p>
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <form action={formAction}>
                  <input type="hidden" name="id" value={facultyId} />
                  <input type="hidden" name="change" value="archive" />
                  <ActionButton label="Archive" pendingLabel="Archiving…" variant="ghost" />
                </form>
                <button
                  type="button"
